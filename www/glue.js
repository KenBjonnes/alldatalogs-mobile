(() => {
  ...
  var Preferences = registerPlugin("Preferences", {
    web: () => Promise.resolve().then(() => (init_web3(), web_exports3)).then((m) => new m.PreferencesWeb())
  });
  ...
  var ScreenOrientation = registerPlugin("ScreenOrientation", {
    web: () => Promise.resolve().then(() => (init_web5(), web_exports5)).then((m) => new m.ScreenOrientationWeb())
  });
  ...
  var KeepAwake = registerPlugin("KeepAwake", {
    web: () => Promise.resolve().then(() => (init_web7(), web_exports7)).then((m) => new m.KeepAwakeWeb())
  });
  ...
  var proxy = registerPlugin("SecureStorage", {
    web: async () => {
      const module = await Promise.resolve().then(() => (init_web8(), web_exports9));
  ...
  async function prefGet(key) {
    const r = await quiet(Preferences.get({ key }), { value: null });
    return r && typeof r.value === "string" ? r.value : null;
  }
  async function prefSet(key, value) {
    await quiet(Preferences.set({ key, value }), void 0);
  }
  async function prefRemove(key) {
    await quiet(Preferences.remove({ key }), void 0);
  }
  var securePrefix = null;
  function securePrefixed() {
    if (!securePrefix) securePrefix = quiet(proxy.setKeyPrefix("bigdata_"), void 0);
    return securePrefix;
  }
  async function secureGet(key) {
    await securePrefixed();
    const v = await quiet(proxy.get(key, false), null);
  ...
  async function secureRemove(key) {
    await securePrefixed();
    await quiet(proxy.remove(key), void 0);
  }
  async function openExternal(url) {
    if (isNative) await quiet(Browser2.open({ url, presentationStyle: "popover" }), void 0);
    else window.open(url, "_blank", "noopener");
  }
  async function lockLandscape() {
    if (!isNative || deviceClass() !== "phone") return;
    await quiet(ScreenOrientation.lock({ orientation: "landscape" }), void 0);
  }
  async function unlockOrientation() {
    if (!isNative) return;
    await quiet(ScreenOrientation.unlock(), void 0);
  }
  async function keepAwake(on) {
    if (!isNative) return;
    await quiet(on ? KeepAwake.keepAwake() : KeepAwake.allowSleep(), void 0);
  }
  async function hideSplash() {
    await quiet(SplashScreen.hide(), void 0);
  }
  async function exitApp() {
    await quiet(App.exitApp(), void 0);
  }
  function onBackButton(cb) {
    if (!isNative) return;
    void quiet(App.addListener("backButton", () => cb()), void 0);
  }
  function onAppActive(cb) {
    void quiet(App.addListener("appStateChange", (s) => cb(!!s.isActive)), void 0);
    if (!isNative) document.addEventListener("visibilitychange", () => cb(document.visibilityState === "visible"));
  }
  function onUrlOpen(cb) {
    void quiet(App.addListener("appUrlOpen", (e) => {
      if (e && e.url) cb(e.url);
    }), void 0);
  }
  async function launchUrl() {
    const r = await quiet(App.getLaunchUrl(), null);
    return r && typeof r.url === "string" && r.url ? r.url : null;
  }
  function onOnline(cb) {
    void quiet(Network2.addListener("networkStatusChange", (s) => {
      if (s.connected) cb();
    }), void 0);
  ...
  async function pickLog() {
    try {
      ...
      if (!f) return null;
      const webPath = f.webPath || (f.path ? Capacitor.convertFileSrc(f.path) : void 0);
  ...
  // src/license.ts
  var DAY_MS = 864e5;
  var REFRESH_EVERY_MS = 6 * 36e5;
  var STALE_AFTER_MS = 6 * 36e5;
  var TAMPER_SLACK_MS = DAY_MS;
  var EXPIRING_DAYS = 7;
  function emptyStore() {
  ...
  function evaluate(input) {
    const { store: store2, verified, now, session, installId: installId2 } = input;
    ...
    const lastResult = store2 && store2.lastResult ? store2.lastResult : null;
    ...
    const withCommon = (s) => ({ ...s, email: s.email || email, storageUnavailable });
    const foreignInstall = !!(claims && claims.mid !== installId2);
    ...
    const highWater = store2 && typeof store2.clockHighWater === "number" ? store2.clockHighWater : 0;
    if (highWater && now < highWater - TAMPER_SLACK_MS) return withCommon(mkState(null, "clock_tamper"));
    if (usable && usable.pro === true && live) {
      const daysLeft = Math.max(0, Math.ceil((usable.exp * 1e3 - now) / DAY_MS));
  ...
  // src/config.ts
  var SUPABASE_URL = "https://nvkfhbfyrbifkbynjbsd.supabase.co";
  var SUPABASE_ANON_KEY = "sb_publishable_fpmjy5vxm416c04llHQ0dQ_oK9t2GZJ";
  var ENTITLEMENT_FUNCTION = "issue-entitlement-token";
  var TOKEN_ISS = "alldatalogs";
  var TOKEN_AUD = "bigdata-mobile";
  var REMOTE_CONFIG_URL = "https://alldatalogs.com/mobile-config.json";
...
Suggested solution: ${env.workaround}`;
  ...
  var RealtimeChannel = class _RealtimeChannel {
    ...
    subscribe(callback, timeout = this.timeout) {
      ...
      if (this.channelAdapter.isClosed()) {
        const { config: { broadcast: broadcast2, presence, private: isPrivate, postgres_changes_options } } = this.params;
        const postgres_changes = (_b = (_a = this.bindings.postgres_changes) === null || _a === void 0 ? void 0 : _a.map((r) => r.filter)) !== null && _b !== void 0 ? _b : [];
        const presence_enabled = !!this.bindings[REALTIME_LISTEN_TYPES.PRESENCE] && this.bindings[REALTIME_LISTEN_TYPES.PRESENCE].length > 0 || ((_c = this.params.config.presence) === null || _c === void 0 ? void 0 : _c.enabled) === true;
        const accessTokenPayload = {};
        const config = Object.assign({ broadcast: broadcast2, presence: Object.assign(Object.assign({}, presence), { enabled: presence_enabled }), postgres_changes, private: isPrivate }, postgres_changes_options ? { postgres_changes_options } : {});
  ...
  var StorageApiError = class extends StorageError2 {
  ...
  var StorageUnknownError = class extends StorageError2 {
  ...
  var StorageAnalyticsClient = class extends BaseApiClient {
    ...
    */
    from(bucketName) {
      var _this4 = this;
      if (!isValidBucketName(bucketName)) throw new StorageError2("Invalid bucket name: File, folder, and bucket names must follow AWS object key naming guidelines and should avoid the use of any other characters.");
  ...
  async function getPKCEFlowIndex(storage2, storageKey) {
    const index = await getItemAsync(storage2, pkceFlowIndexKey(storageKey));
  ...
  async function storePKCEVerifier(storage2, storageKey, flowId, verifier, onEvictFlow) {
    await setItemAsync(storage2, pkceVerifierSlotKey(storageKey, flowId), verifier);
    ...
    while (index.length > PKCE_MAX_CONCURRENT_FLOWS) {
      ...
      await removeItemAsync(storage2, pkceVerifierSlotKey(storageKey, evicted));
      onEvictFlow === null || onEvictFlow === void 0 ? void 0 : onEvictFlow(evicted);
    }
    await setItemAsync(storage2, pkceFlowIndexKey(storageKey), index);
    await setItemAsync(storage2, `${storageKey}-code-verifier`, verifier);
  }
  async function retrievePKCEVerifier(storage2, storageKey, flowId) {
    if (flowId) {
      const verifier2 = await getItemAsync(storage2, pkceVerifierSlotKey(storageKey, flowId));
      return { verifier: typeof verifier2 === "string" ? verifier2 : null, flowId };
    }
    const verifier = await getItemAsync(storage2, `${storageKey}-code-verifier`);
  ...
  async function removePKCEVerifier(storage2, storageKey, flowId) {
    ...
    if (!flowId) {
      await removeItemAsync(storage2, legacyKey);
      return;
    }
    const slotKey = pkceVerifierSlotKey(storageKey, flowId);
    const slotValue = await getItemAsync(storage2, slotKey);
    await removeItemAsync(storage2, slotKey);
    ...
    if (remaining.length !== index.length) {
      if (remaining.length > 0) {
        await setItemAsync(storage2, pkceFlowIndexKey(storageKey), remaining);
      } else {
        await removeItemAsync(storage2, pkceFlowIndexKey(storageKey));
      }
    }
    if (slotValue != null && slotValue === await getItemAsync(storage2, legacyKey)) {
      await removeItemAsync(storage2, legacyKey);
    }
  }
  async function removeAllPKCEVerifiers(storage2, storageKey) {
    ...
    for (const flowId of index) {
      await removeItemAsync(storage2, pkceVerifierSlotKey(storageKey, flowId));
    }
    await removeItemAsync(storage2, pkceFlowIndexKey(storageKey));
    await removeItemAsync(storage2, `${storageKey}-code-verifier`);
  }
...
${suffix}`;
  ...
  var GoTrueClient = class _GoTrueClient {
    ...
    async _notifyAllSubscribers(event, session, broadcast2 = true) {
      if (this._pendingInitNotifications !== null && broadcast2) {
        this._pendingInitNotifications.push({ event, session, broadcast: broadcast2 });
        return;
      }
  ...
  function sb() {
    if (!client) {
      client = createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
        ...
        global: { headers: { "x-client-info": `bigdata-mobile-${platform}/${"0.1.0"}` } }
      });
    }
    return client;
  }
  function isSecureStorageOk() {
    return secureOk;
  }
  ...
  async function fetchEntitlementToken(accessToken, installId2) {
    const url = `${SUPABASE_URL}/functions/v1/${ENTITLEMENT_FUNCTION}`;
    let res;
    try {
      const ctl = new AbortController();
      const t = setTimeout(() => ctl.abort(), 2e4);
      res = await fetch(url, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${accessToken}`, apikey: SUPABASE_ANON_KEY },
        body: JSON.stringify({ installId: installId2, appVersion: "0.1.0", client: "mobile", platform }),
        signal: ctl.signal
      }).finally(() => clearTimeout(t));
    } catch (e) {
      return { kind: "network", error: e?.message || "network" };
    }
    let body = null;
    try {
      body = await res.json();
  ...
  async function deleteAccount() {
    try {
      ...
      return data && data.ok ? { ok: true } : { ok: false, error: data?.error || "Could not delete the account." };
    } catch (e) {
  ...
  async function pullLayouts() {
    ...
    return { ok: true, rows: data.map((r) => ({ id: r.id, name: r.name, state: r.state, updatedAt: r.updated_at ? Date.parse(r.updated_at) : Date.now() })) };
  }
  ...
  var LIB_COLS = "id,kind,name,description,owner_id,author_name,is_official,status,thumb_svg,vehicle,vehicle_year,vehicle_make,vehicle_model,tags,pulls,created_at,updated_at";
  var likeTerm = (q) => String(q || "").replace(/[,()%\\]/g, " ").trim().slice(0, 60);
  function vehicleCols(v) {
    ...
    return {
      ...
      vehicle_year: y && y >= 1900 && y <= 2100 ? y : null,
  ...
  function buildLibraryProvider(opts) {
    let adminKnown = null;
    return {
      async list(q = {}) {
        const size = Math.max(1, Math.min(60, Number(q.pageSize) || 30));
        const from = Math.max(0, Number(q.page) || 0) * size;
        let req = sb().from("library_items").select(LIB_COLS).order("is_official", { ascending: false }).order("pulls", { ascending: false }).order("updated_at", { ascending: false }).range(from, from + size);
        ...
        if (error || !data) return { items: [], hasMore: false, error: error?.message };
      ...
      async publish(input) {
        ...
        const row = {
          ...
          thumb_svg: typeof input.thumb_svg === "string" ? input.thumb_svg.slice(0, 6e4) : null,
          ...vehicleCols(input),
        ...
        if (await this.isAdmin()) {
          ...
          if (r && r.ok) item.is_official = true;
        }
        return { ok: true, item };
      },
      ...
      async update(id, input) {
        ...
        if (typeof input.name === "string") {
          ...
          if (!patch.name) return { ok: false, error: "Give it a name first." };
        }
        if (typeof input.description === "string") patch.description = input.description.trim().slice(0, 2e3);
        if (input.payload && typeof input.payload === "object") patch.payload = input.payload;
        if (input.thumb_svg !== void 0) patch.thumb_svg = typeof input.thumb_svg === "string" ? input.thumb_svg.slice(0, 6e4) : null;
        if (input.vehicle !== void 0 || input.vehicle_make !== void 0 || input.vehicle_model !== void 0 || input.vehicle_year !== void 0) Object.assign(patch, vehicleCols(input));
      ...
      async pull(id) {
        try {
          await sb().rpc("library_pull", { item_id: String(id) });
        } catch {
        }
      },
      async me() {
      ...
      admin: {
        setOfficial: (id, value) => adminCall("library_official", { itemId: id, value }),
        hide: (id, value) => adminCall("library_hide", { itemId: id, value }),
        remove: (id) => adminCall("library_delete", { itemId: id })
      }
    };
  }

  // src/licensing.ts
  var STORE_KEY = "bigdata.license.v1";
  var INSTALL_KEY = "bigdata.installId";
  var DEV_PRO = !isNative && platform === "web" && /[?&]devpro=1/.test(location.search);
  var store = emptyStore();
  var installId = "";
  var state = DEV_PRO ? { pro: true, reason: "dev", status: "active", daysLeft: null, email: "dev@localhost", storageUnavailable: false } : { pro: null, reason: "signed_out", status: null, daysLeft: null, email: null, storageUnavailable: false };
  var refreshing = null;
  var inFlight = false;
  ...
  async function readStore() {
    try {
      const o = JSON.parse(await prefGet(STORE_KEY) || "null");
      if (o && typeof o === "object") return { ...emptyStore(), ...o };
    } catch {
    }
    return emptyStore();
  }
  async function writeStore() {
    await prefSet(STORE_KEY, JSON.stringify(store));
  }
  async function loadInstallId() {
    const v = await prefGet(INSTALL_KEY) || "";
    if (/^[a-f0-9-]{36}$/i.test(v)) return v;
    const id = crypto && "randomUUID" in crypto ? crypto.randomUUID() : "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, (c) => {
  ...
  async function verifyStored(now) {
    ...
    try {
      return await verifyJws(store.token, { keys: ENTITLEMENT_KEYS, iss: TOKEN_ISS, aud: TOKEN_AUD, now: Math.floor(now / 1e3) });
    } catch {
      return { ok: false, error: "malformed" };
    }
  }
  ...
  async function recompute() {
    ...
    const now = Date.now();
    if (now > (store.clockHighWater || 0)) {
      store.clockHighWater = now;
      await writeStore();
    }
  ...
  async function refresh() {
    ...
    refreshing = (async () => {
      try {
        ...
        const now = Date.now();
        if (r.kind === "ok") {
          const v = await verifyJws(r.token, { keys: ENTITLEMENT_KEYS, iss: TOKEN_ISS, aud: TOKEN_AUD, now: Math.floor(now / 1e3) });
          if (v.ok && v.claims.mid === installId && v.claims.sub === session.userId) {
            store.token = r.token;
            store.fetchedAt = now;
            store.clockHighWater = Math.max(now, (Number(v.claims.iat) || 0) * 1e3);
  ...
  async function signIn2(email, password) {
    ...
    const r = await signIn(String(email || "").trim(), String(password || ""));
    if (!r.ok) return r;
    await refresh();
    return { ok: true };
  }
  ...
  function capBytes(opts) {
    ...
    const mb = opts.pro ? (opts.device === "tablet" ? caps2.tablet : caps2.phone)[opts.fmt] ?? caps2.free : caps2.free;
    return Math.max(1, Math.round(mb * 1048576));
  }
  function fullBudgetCells(device2, caps2) {
    const c = caps2 || DEFAULT_CAPS;
    return device2 === "tablet" ? c.fullCellsTablet : c.fullCellsPhone;
  }
  function mergeCaps(remote2) {
    const r = remote2 && typeof remote2 === "object" ? remote2 : {};
    const num = (v, d) => typeof v === "number" && isFinite(v) && v > 0 ? v : d;
    const table = (v, d) => {
      const out = { ...d };
      if (v && typeof v === "object") for (const k of Object.keys(d)) out[k] = num(v[k], d[k]);
      return out;
    };
    return {
  ...
  // src/files.ts
  var RECENTS_KEY = "bigdata.recents.v1";
  var RECENTS_MAX = 12;
  ...
  async function openFromUrl(url, origin = "open-with", nameHint) {
  ...
  var remote = {};
  var license = getState();
  var log = [];
  function note(s) {
    log.push((/* @__PURE__ */ new Date()).toISOString().slice(11, 19) + " " + s);
  ...
  var layoutProvider = {
    ...
    save(state2, name) {
      ...
      const entry = saveLayout(trimmed, state2);
      if (isPro()) pushLayout(entry).catch(() => {
      });
      return true;
    },
    apply(id) {
  ...
  var pending = null;
  var cancelled = false;
  var OPEN_MARK = "bigdata.opening";
  function openParsed(parsed, src) {
  ...
  async function runSyncJob(job) {
    ...
    let ab;
    try {
      ab = await job.src.file.arrayBuffer();
    } catch {
  ...
  function getWorker() {
    ...
    try {
      ...
      w.onerror = (ev) => {
        note("worker error: " + (ev && ev.message));
        workerBroken = true;
        dropWorker();
        const p = pending;
        pending = null;
      ...
      note("worker unavailable: " + errMsg(e, "?"));
      workerBroken = true;
      return null;
    }
  }
  function startJob(src) {
    cancelled = false;
    const id = ++jobSeq, job = { id, src, fmt: fmtOf(src.name) };
    clearError();
    showLoader(src.name);
    void prefSet(OPEN_MARK, JSON.stringify({ name: src.name, size: src.size, at: Date.now() }));
    const w = getWorker();
    if (!w) {
      void runSyncJob(job);
      return;
    ...
    pending = job;
    try {
      w.postMessage({ jobId: id, name: src.name, fmt: job.fmt, file: src.file });
    } catch (e) {
      note("postMessage failed: " + errMsg(e, "?"));
      workerBroken = true;
      dropWorker();
      pending = null;
      void runSyncJob(job);
  ...
  function handleSource(src) {
    const fmt = fmtOf(src.name), pro = isPro();
    const cap = capBytes({ pro, device, fmt, caps });
    if (src.size > cap) {
      const why = pro ? `This ${fmt} log is ${fmtBytes(src.size)}; the limit on a ${device} is ${fmtBytes(cap)} so it can be opened without running out of memory. Open it on the website or BigData for Windows.` : `This log is ${fmtBytes(src.size)}, above the free limit of ${fmtBytes(cap)}. AllDataLogs Pro opens logs up to ${fmtBytes(capBytes({ pro: true, device, fmt, caps }))} on this ${device}.`;
      showError(why);
      return;
    }
    startJob(src);
  }
  async function openWith(fn) {
    try {
      const src = await fn();
      if (src) handleSource(src);
    } catch (e) {
      showError(errMsg(e, "Could not open that file."));
    }
  }
  var booted = false;
  var queuedUrls = [];
  function openUrl(url) {
    if (!url || /^(https?|bigdata|alldatalogs):/i.test(url)) return;
    if (!booted) {
      queuedUrls.push(url);
      return;
    }
  ...
  function showScreen(name) {
    screen2 = name;
    for (const s of ["home", "account", "diag"]) $(`screen-${s}`).hidden = s !== name;
    if (name === "diag") renderDiag();
    if (name === "account") renderAccount();
  }
  function showLoader(name) {
    $("loaderName").textContent = `Opening ${name}\u2026`;
    $("loader").hidden = false;
  }
  function hideLoader() {
    $("loader").hidden = true;
  ...
  async function refreshRecents() {
    ...
    for (const r of rows) {
      ...
      li.addEventListener("click", () => {
        if (!r.uri) {
          window.showToast("Open it again from your files or the share sheet.");
          return;
        }
        void openWith(() => openFromUrl(r.uri, "recent", r.name)).catch(() => {
        });
      });
      ul.appendChild(li);
    }
  ...
  function planLine(s) {
    if (s.pro === true) return s.reason === "ok" ? `AllDataLogs Pro \xB7 active${s.daysLeft != null ? ` \xB7 verified, ${s.daysLeft} days of offline grace` : ""}` : "AllDataLogs Pro \xB7 " + s.reason.replace(/_/g, " ");
    if (s.pro === false) return s.reason === "wrong_account" ? "This account is not the one Pro was activated for on this device." : `Free \xB7 no active Pro membership${s.status && s.status !== "none" ? ` (${s.status})` : ""}`;
  ...
  function applyLicense(s) {
    ...
    for (const id of ["banner", "accountBanner"]) {
      const el = $(id);
      if (b) {
        el.textContent = b.text(s);
        el.classList.toggle("bad", !!b.bad);
        el.hidden = false;
      } else el.hidden = true;
    }
    ...
    pro.textContent = s.pro === true ? s.reason === "ok" ? "Pro" : "Pro \xB7 " + s.reason.replace(/_/g, " ") : s.email ? "Free" : "";
    $("btnAccount").textContent = s.email ? "Account" : "Sign in";
    capNote();
    if (screen2 === "account") renderAccount();
  }
  function proLinkAllowed() {
  ...
  function renderAccount() {
    ...
    document.querySelectorAll("[data-pro-link]").forEach((el) => {
      el.hidden = !proLinkAllowed();
    });
    if (!signedIn) {
      $("signinIntro").textContent = license.reason === "activate_offline" ? "Connect to the internet and sign in so BigData can check your membership." : "Sign in with your alldatalogs.com account to unlock Pro on this device.";
      return;
    }
  ...
  function renderDiag() {
    ...
    $("diagText").textContent = [
      `BigData ${"0.1.0"} \xB7 ${platform} \xB7 ${device} \xB7 ${window.screen.width}x${window.screen.height} @${window.devicePixelRatio}`,
      `viewport ${window.innerWidth}x${window.innerHeight} \xB7 online ${navigator.onLine} \xB7 worker ${workerBroken ? "BROKEN (sync fallback)" : "ok"}`,
      `caps ${JSON.stringify(caps)}`,
      `remote ${JSON.stringify(remote)} \xB7 storefront ${window.__bigdata.storefront || "?"}`,
      `licence ${JSON.stringify(snap.state)}`,
      `store ${JSON.stringify(snap.store)} \xB7 install ${snap.installId}`,
      `engine ${typeof window.VIEWER_VERSION !== "undefined" ? window.VIEWER_VERSION : "?"} \xB7 DVCore ${!!window.DVCore} \xB7 library ${!!window.Library}`,
      "",
      "log:",
      ...log.slice(-60)
    ].join("\n");
  ...
  function wire() {
    document.querySelectorAll("[data-ext]").forEach((el) => el.addEventListener("click", () => {
      void openExternal(el.dataset.ext || "");
    }));
    document.querySelectorAll("[data-nav]").forEach((el) => el.addEventListener("click", () => showScreen(el.dataset.nav || "home")));
    $("btnOpen").addEventListener("click", () => {
      void openWith(openFromPicker);
    });
    $("btnSample").addEventListener("click", () => {
      void openWith(openSample);
    });
    ...
    $("btnRefresh").addEventListener("click", async () => {
      applyLicense(await refresh());
      window.showToast("Membership checked.");
    });
    $("btnSignOut").addEventListener("click", async () => {
      await signOut2();
      applyLicense(getState());
      showScreen("home");
    });
    $("btnDeleteAccount").addEventListener("click", async () => {
      ...
      if (!r.ok) {
        window.showToast(r.error || "Could not delete the account.");
        return;
      }
      await signOut2();
      applyLicense(getState());
      showScreen("home");
      window.showToast("Your account has been deleted.");
    });
    const form = $("signinForm"), err = $("signinError"), btn = $("signinBtn");
    form.addEventListener("submit", async (e) => {
      e.preventDefault();
      err.hidden = true;
      btn.disabled = true;
      try {
        const r = await signIn2($("signinEmail").value.trim(), $("signinPassword").value);
        if (!r.ok) {
    ...
    $("footVersion").textContent = "v0.1.0";
    onBackButton(() => {
      if (viewerIsOpen()) {
        window.closeViewer?.();
        return;
    ...
    window.addEventListener("unhandledrejection", (e) => note("rejection: " + errMsg(e.reason, "?")));
  }
  async function loadRemoteConfig() {
    try {
      ...
      if (j && typeof j === "object") {
        remote = { proLinkIOS: !!j.proLinkIOS, minVersion: j.minVersion, message: j.message };
        if (j.caps) caps = mergeCaps(j.caps);
        capNote();
      }
    } catch {
    }
  ...
  async function oomCheck() {
    const raw = await prefGet(OPEN_MARK);
    if (!raw) return;
    await prefRemove(OPEN_MARK);
    try {
      const m = JSON.parse(raw);
      note("previous open did not finish: " + m.name);
      showError(`BigData ran out of memory opening ${m.name} (${fmtBytes(m.size || 0)}) last time. Try a smaller log, or open it on the website or BigData for Windows.`);
    } catch {
    }
  }
  async function boot() {
    window.__bigdata = { version: "0.1.0", platform, device, storefront: null };
    window.configureViewer({ brand: { mark: "B", name: "BigData", sub: "DATALOG VIEWER" }, layouts: layoutProvider, library: buildLibraryProvider({ isPro: () => isPro() }) });
